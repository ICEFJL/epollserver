...
void ngx_master_process_cycle() {
  ...
  if (sigprocmask(SIG_BLOCK, &set, NULL) ==
      -1)  // 第一个参数用了SIG_BLOCK表明设置 进程 新的信号屏蔽字 为
           // “当前信号屏蔽字 和 第二个参数指向的信号集的并集
  {
  ...
  int i;
  size = sizeof(
      master_process);  // 注意我这里用的是sizeof，所以字符串末尾的\0是被计算进来了的
  ...
  if (size < 1000)        // 长度小于这个，我才设置标题
  {
    ...
    ngx_log_error_core(
        NGX_LOG_NOTICE, 0, "%s %P 【master进程】启动并开始运行......!", title,
  ...
  int workprocess = p_config->GetIntDefault(
      "WorkerProcesses", 1);  // 从配置文件中得到要创建的worker进程数量
...
static void ngx_start_worker_processes(int threadnums) {
  int i;
  for (i = 0; i < threadnums;
       i++)  // master进程在走这个循环，来创建若干个子进程
  {
...
static void ngx_worker_process_cycle(int inum, const char *pprocname) {
  ...
  ngx_log_error_core(
      NGX_LOG_NOTICE, 0, "%s %P 【worker进程】启动并开始运行......!", pprocname,
      ngx_pid);  // 设置标题时顺便记录下来进程名，进程id等信息到日志
...
static void ngx_worker_process_init(int inum) {
  ...
  int tmpthreadnums = p_config->GetIntDefault(
      "ProcMsgRecvWorkThreadCount", 5);  // 处理接收到的消息的线程池中线程数量
  ...
  g_socket
      .ngx_epoll_init();  // 初始化epoll相关内容，同时
